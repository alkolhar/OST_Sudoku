 ...
 *
 */
public class SudokuSingles {
	int row;
	int column;
	int[][] board = new int[9][9];

	// creates an initially empty board
	public SudokuSingles() {
	...
	// creates an initial board defined by board[r][c]. Use 0 to represent an empty
	// cell.
	public SudokuSingles(int[][] board) {
	...
	public boolean isSolved() {
		...
		for (int i = 0; i < board.length; i++) {
			for (int j = 0; j < board.length; j++) {
				if (board[i][j] == 0)
					return false;
				else if (i == 8 || i == 17 || i == 26 || i == 35 || i == 44 || i == 53 || i == 62 || i == 71
						|| i == 80) {
					if (i == 8)
						bit = s.substring(0, i);
					else
	...
	// attempts to solve the board. Exits when it is solved or no updates were made
	// to the board.
	public void solve() {
	...
	// strategy to find cells that have a unique candidate for their subunit (row,
	// column, or box)
